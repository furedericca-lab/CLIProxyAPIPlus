...
const (
	...
	cloudflareBackoffBase     = 10 * time.Second
	cloudflareBackoffMax      = 30 * time.Minute
)
...
func (m *Manager) executeMixedOnce(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, maxRetryCredentials int) (cliproxyexecutor.Response, error) {
	...
	for {
		...
		for _, upstreamModel := range models {
			...
			execOpts := opts
			execReq, execOpts = applyRequestAfterAuthInterceptor(attemptCtx, executor, provider, execReq, execOpts, requestedModelAliasFromOptions(execOpts, routeModel))
...
func nextCloudflareCooldown(backoffLevel int, disableCooling bool, now time.Time) (time.Time, int) {
	...
	if !disableCooling {
		...
		nextLevel := backoffLevel + 1
		if cooldown < cloudflareBackoffBase {
			cooldown = cloudflareBackoffBase
		}
		if cooldown >= cloudflareBackoffMax {
			cooldown = cloudflareBackoffMax
			nextLevel = backoffLevel
		}
